...
class ApiDocTest extends DocTester {
    ...
    @AfterAll
    static void afterAll() { finishDocTest(); }
...
\section{DocTester Architecture}
...
\section{Java 26 Features in DocTester}

DocTester is not merely a framework written in Java; its architecture is significantly strengthened by modern Java features. Records, sealed classes, pattern matching for switch, virtual threads, and sequenced collections make the design substantially cleaner, safer, and more verifiable than older Java idioms would allow. Each of the features listed below is load-bearing: removing it would require either a materially worse API or a fundamentally different architecture.
...
\textbf{Records} (JEP 395, final in Java 16) provide the right representation for event types: immutable, structurally typed, with auto-generated accessors, equality, and string representation. Every SayEvent subtype is a record. The component list IS the class — there is nothing else to maintain.
...
\textbf{Virtual threads} (JEP 444, final in Java 21) make the `MultiRenderMachine` design trivially correct. Each render machine receives every event in a separate virtual thread. Virtual threads are JVM-scheduled lightweight coroutines — creating one per render machine per say* call costs \textasciitilde{}1 KB RAM and near-zero CPU overhead. Sequential rendering scales linearly with the number of target formats in wall-clock time. Virtual-thread-based dispatch does not change the total amount of work, but it can substantially reduce wall-clock latency by allowing independent renderers to proceed concurrently with very low thread-management overhead.
\cite{OpenJDK2024}
...
\section{AI-Augmented Development with Claude Code}
...
\texttt{Documentation drift incidents} & \texttt{0 observed in examples; executable claims are structurally constrained} \\
...
\begin{tcolorbox}[colback=notebg,colframe=blue!50!black,title={\textbf{Note}}]
The thesis itself was generated by a Claude Code session. Every say* call in this file was authored by Claude Sonnet 4.6. The citations were registered in @BeforeAll. The chapter ordering is enforced by @TestMethodOrder(MethodName). This thesis is an example of AI-augmented academic software writing in which a human author defined the thesis direction and an AI system materially contributed to drafting and code-oriented exposition.
\end{tcolorbox}
...
\section{Blue Ocean Innovations}
...
\textbf{Innovation 1: sayCallSite() — Call-Site Provenance via StackWalker}
...
StackWalker.getInstance(StackWalker.Option.RETAIN\_CLASS\_REFERENCE)
    .walk(frames -> frames
...
\textbf{Innovation 2: sayAnnotationProfile() — Complete Annotation Landscape}
...
\textbf{Innovation 3: sayClassHierarchy() — Inheritance Tree, Auto-Generated}
...
\textbf{Innovation 4: sayStringProfile() — Structural Analysis of String Payloads}
...
\textbf{Innovation 5: sayReflectiveDiff() — Field-by-Field Comparison, Zero Boilerplate}
...
\textbf{Implications for software engineering practice.} If the test-as-documentation paradigm is adopted broadly, it requires a change in how development teams think about the relationship between testing and documentation. Currently, testing and documentation are separate activities in most team's process: developers write tests, technical writers write documentation, and the two artifacts are reviewed separately. DocTester proposes that these activities are fundamentally the same activity performed with a different output format selected.
...
This thesis has presented DocTester as a canonical demonstration that the documentation drift problem is structurally solvable. The solution is not a better documentation tool, a better synchronization process, or a more disciplined team. The solution is the elimination of the structural separation between documentation and tests. When the test and the document are derived from the same executable artifact, a large and costly class of documentation drift becomes mechanically detectable rather than socially managed.
...
\bibitem{Fowler1999}
Martin Fowler. \textit{Refactoring: Improving the Design of Existing Code}. Addison-Wesley, 1999.

\bibitem{Beck2002}
Kent Beck. \textit{Test-Driven Development: By Example}. Addison-Wesley, 2002.

\bibitem{North2006}
Dan North. Introducing BDD, 2006.

\bibitem{Richardson2007}
Leonard Richardson and Sam Ruby. \textit{RESTful Web Services}. O'Reilly, 2007.

\bibitem{Freeman2009}
Steve Freeman and Nat Pryce. \textit{Growing Object-Oriented Software, Guided by Tests}. Addison-Wesley, 2009.

\bibitem{Humble2010}
Jez Humble and David Farley. \textit{Continuous Delivery}. Addison-Wesley, 2010.

\bibitem{Gosling2023}
James Gosling et al. \textit{The Java Language Specification, Java SE 21 Edition}. Oracle, 2023.

\bibitem{JEP431}
OpenJDK. JEP 431: Sequenced Collections, 2023.

\bibitem{JEP441}
OpenJDK. JEP 441: Pattern Matching for switch, 2023.

\bibitem{JEP444}
OpenJDK. JEP 444: Virtual Threads, 2023.

\bibitem{JEP445}
OpenJDK. JEP 445: Unnamed Classes and Instance Main Methods, 2023.

\bibitem{JEP456}
OpenJDK. JEP 456: Unnamed Variables and Patterns, 2024.

\bibitem{JEP488}
OpenJDK. JEP 488: Primitive Types in Patterns, instanceof, and switch, 2024.

\bibitem{Anthropic2024}
Anthropic. Technical report, 2024.

\bibitem{OpenJDK2024}
OpenJDK. Project Loom documentation, 2024.
