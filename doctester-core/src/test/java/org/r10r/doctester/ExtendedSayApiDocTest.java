...
class ExtendedSayApiDocTest extends DocTester {
    ...
    void documentJava26FeatureMap() {
        sayNextSection("Java 26 Feature Map — Every Modern Idiom in DocTester");

        say("DocTester's architecture is a canonical demonstration of Java 25/26 features. "
                + "The sealed SayEvent hierarchy, record-based event types, virtual thread dispatch, "
                + "and reflection-based code model all work together as a coherent system.");

        sayTable(new String[][] {
            {"Java 26 Feature",           "Where in DocTester",               "Why"},
            {"Sealed classes",            "SayEvent interface",               "Type-safe event dispatch"},
            {"Records",                   "All 16 SayEvent subtypes",         "Immutable event carriers"},
            {"Pattern matching switch",   "MultiRenderMachine.dispatchToAll", "No visitor pattern needed"},
            {"Virtual threads",           "MultiRenderMachine dispatch",      "Concurrent multi-format output"},
            {"Unnamed patterns (_)",      "Render switches",                  "Ignore unused components"},
            {"Sequenced collections",     "Event pipeline (LinkedList)",      "Ordered, first-class access"},
            {"Code model (Babylon)",      "sayCodeModel() method",            "Docs derived from bytecode"},
            {"Text blocks",               "HTML/LaTeX templates",             "Template as output"},
            {"var inference",             "Throughout streams/loops",         "Obvious RHS → infer type"},
        });

        say("The SayEvent sealed interface is the heart of the event system. "
                + "Every say* method call produces a SayEvent that every renderer handles:");

        sayCodeModel(SayEvent.class);

        say("The TextEvent record is the simplest example of a record in DocTester:");

        sayCodeModel(SayEvent.TextEvent.class);

        sayNote("These sections were generated by reflection from the live bytecode. "
                + "They cannot drift from the implementation. When the class changes, the doc changes.");
    }
