...
// Falls back gracefully with a text note on Java 25 runtimes
...
- `sayControlFlowGraph` and `sayCallGraph` use the Java 26 Code Reflection API
  (`java.lang.reflect.code`) where available, with graceful fallback on Java 25.
...
DTR 2.5.0 is the **Maven Central Ready** release. It stabilizes Java 25 support,
...
- **Javadoc JAR** — `dtr-core-2.5.0-javadoc.jar` generated by `maven-javadoc-plugin 3.11.2`
  with `doclint:none` to allow Java 25 preview feature documentation.
...
#### 2. Java 25.0.2+ with `--enable-preview` Now Enforced

Enforced by `maven-enforcer-plugin`:
```xml
<requireJavaVersion>
  <version>[25,)</version>
  <message>Java 25 or higher is required for DTR 2.5.0.</message>
</requireJavaVersion>
```
...
- **Jetty 9.4.x + Java 25:** Jetty 9.4.x may fail to start with `--enable-preview`
...
- **325 tests, 0 failures** with Java 25 preview flags enabled
...
- All code examples updated to Java 25 idioms (records, pattern matching, text blocks)
...
`MultiRenderMachine` dispatches all events to registered machines in parallel via Java 25
virtual threads.
...
On Java 25, falls back to `Class#getDeclaredMethods()`. Graceful, transparent degradation.
...
- Java 26+: operation breakdown from Code Reflection IR (INVOKE count, FIELD_READ count,
  BRANCH count, etc.)
- Java 25-: method signature only (parameters with types + return type via reflection)
...
Java 25 virtual threads. Eliminates wall-clock latency when producing multiple output
...
- Minimum Java version formally stated as **Java 25 LTS with `--enable-preview`** in
...
- `maven-enforcer-plugin` rules validating Java 25 + Maven 4.0.0-rc-5+
...
JUnit 4 inheritance to JUnit 5 extension, Java 25 becomes the mandatory runtime, and
...
**Key theme:** Complete modernization — Markdown-first, Java 25, JUnit 5, Maven 4.
...
The entire codebase was modernized to use Java 25 idioms:
...
- Enhanced enforcer rules validating Java 25 + Maven 4 combination
...
#### Java Requirement: 1.8+ → Java 25 LTS (Enforced)
...
- **JAXB module issues on Java 25+:** JAXB was removed from the JDK in Java 11. v1.x
  tests using XML binding failed with `NoClassDefFoundError` on Java 25. Fixed by
...
| 3 | Java 1.8+ → Java 25 required | All users | CI/CD runners, JAVA_HOME |
...
- [ ] Install Java 25 (SDKMAN: `sdk install java 25.0.2-open`)
...
- [ ] Update `maven-enforcer-plugin` rules for Java 25 + Maven 4
