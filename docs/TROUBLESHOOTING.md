...
**Cause:** Using Java 25 or earlier with DTR 2026.3.0+.
...
**New (2026.3.0+):** `DtrTest` still exists, but `DtrExtension` + `DtrContext` is preferred

**Migration Path:**
```bash
# 1. Update dependency version
sed -i 's/<version>2\.[0-9]\.[0-9]<\/version>/<version>2026.3.0<\/version>/' pom.xml
