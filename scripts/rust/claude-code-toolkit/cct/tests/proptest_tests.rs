...
fn prop_hash_streaming_equivalence() {
    proptest!(|(chunk1 in ".*", chunk2 in ".*", chunk3 in ".*")| {
        ...
        // Verify that direct hashing of same input is deterministic
        let hash_direct2 = cct_cache::hasher::hash_method_body(full_input.as_bytes());

        prop_assert_eq!(
            hash_direct, hash_direct2,
